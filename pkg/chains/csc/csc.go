// Package csc provides utilities for CSC - CasinoCoin.
package csc

import (
	"encoding/hex"
	"log"

	"github.com/eknkc/basex"
)

var (
	EncodingCSC *basex.Encoding
	EncodingBTC *basex.Encoding
)

func init() {
	var err error
	EncodingCSC, err = basex.NewEncoding("cpshnaf39wBUDNEGHJKLM4PQRST7VWXYZ2brdeCg65jkm8oFqi1tuvAxyz")
	if err != nil {
		log.Fatal(err)
	}

	EncodingBTC, err = basex.NewEncoding("123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz")
	if err != nil {
		log.Fatal(err)
	}
}

// ConvertAddr converts legacy btc address to CSC address
func ConvertAddr(s string) (addr string, err error) {
	var b []byte
	b, err = EncodingBTC.Decode(s)
	if err != nil {
		return
	}
	addr = EncodingCSC.Encode(b)

	return
}

// ConvertPriv converts wif private key to CSC private key
func ConvertPriv(s string) (priv string, err error) {
	var b []byte
	b, err = EncodingBTC.Decode(s)
	if err != nil {
		return
	}
	priv = hex.EncodeToString(b)[2:66]

	return
}
